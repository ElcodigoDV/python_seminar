...
import random
n = int(input("n  кол-во первого мн-ва: "))
m = int(input("m  кол-во второго мн-ва: "))
set_1 = set(random.randint(1, 10) for i in range(n))
print(set_1)
set_2 = set(random.randint(1, 10) for i in range(n))
print(set_2)
intersec = sorted(set_1.intersection(set_2))
print(*intersec)
...
while (i < kust):
    if (i == kust - 1):
        max_sum = yagoda[i] + yagoda[i - 1] + yagoda[0]
    else:
        max_sum = yagoda[i] + yagoda[i - 1] + yagoda[i + 1]
        result.append(max_sum)
        result.sort()
    i += 1

print(f"макс число ягод за один проход {result[-1]}")
